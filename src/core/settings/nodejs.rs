...
pub fn download_and_install_nodejs_from_url(
    ...
    download_and_install_nodejs_from_url_cancellable(url, progress_sender, None)
}

/// 使用指定 URL 下载并安装 Node.js，可通过取消令牌安全停止当前任务。
pub fn download_and_install_nodejs_from_url_cancellable(
    url: &str,
    progress_sender: Option<Sender<EnvInstallProgress>>,
...

    {
        ...
        loop {
                ...
                .is_some_and(|flag| flag.load(Ordering::Relaxed))
            {
                ...
                return Err(io::Error::new(io::ErrorKind::Interrupted, "Node.js 安装已取消").into());
            }
    ...
    for i in 0..total_files {
        ...
        {
            drop(archive);
            let _ = fs::remove_dir_all(&nodejs_dir);
            let _ = fs::remove_file(&temp_file_path);
            return Err(io::Error::new(io::ErrorKind::Interrupted, "Node.js 安装已取消").into());
        }
