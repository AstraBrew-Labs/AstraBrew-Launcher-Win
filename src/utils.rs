...
/// 默认酒馆 WebUI 配置文件内容（优化过的默认设置）。
///
/// 首次启动酒馆时，该内容会被复制到酒馆的 `data/default-user/settings.json`。
/// `currentVersion` 字段会在复制前自动更新为当前酒馆实例版本。
///
/// 如果需要修改该文件，只需修改此常量即可，无需手动维护物理文件。
pub const TEMPLATE_TAVERN_SETTINGS_JSON: &str = r#####"{
    "firstRun": false,
    "accountStorage": {
        "__migrated": "1",
        "LNavOpened": "true",
...
/// 把字节数格式化成人类可读的容量文本（`1.23 GB` 这类）。
///
/// 与磁盘容量、下载体积等展示共用，避免各处各写一份换算逻辑。
...
impl AppPaths {
    ...
    /// 当前 SillyTavern 实时日志。
    pub fn sillytavern_log_file(&self) -> PathBuf {
        self.logs.join("sillytavern.log")
    }

    /// 上一次 SillyTavern 启动会话日志。
    pub fn sillytavern_latest_log_file(&self) -> PathBuf {
    ...
    /// 默认酒馆设置模板目录: `root/data/default/sillytavern/`
    pub fn default_tavern_settings_dir(&self) -> PathBuf {
        self.data.join("default").join("sillytavern")
    }

    /// 默认酒馆 WebUI 设置模板文件: `root/data/default/sillytavern/settings.json`
    pub fn default_tavern_settings_file(&self) -> PathBuf {
        self.default_tavern_settings_dir().join("settings.json")
    }

    /// 确保默认酒馆 settings.json 模板文件与常量一致。
    ///
    /// 模板文件始终从 [`TEMPLATE_TAVERN_SETTINGS_JSON`] 常量覆写，
    /// 确保常量更新后模板文件自动同步，不会残留旧内容。
    pub fn ensure_default_tavern_settings(&self) {
        let file = self.default_tavern_settings_file();
