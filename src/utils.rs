...
impl AppPaths {
    ...
    /// 全局模式酒馆 WebUI 设置目标文件:
    /// `root/data/sillytavern/data/default-user/settings.json`
    pub fn global_tavern_settings_file(&self) -> PathBuf {
        self.default_global_data_dir().join("default-user").join("settings.json")
    }
